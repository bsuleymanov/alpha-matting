...
class PVTBackbone(BaseBackbone):
    ...
    def __init__(self, in_channels):
        ...
        self.model = PyramidVisionTransformer(
            patch_size=2, embed_dims=[64, 64, 128, 256, 512], n_heads=[1, 2, 4, 8, 8], mlp_ratios=[8, 8, 4, 4, 4],
...
class ResNet18Backbone(BaseBackbone):
    ...
    def forward(self, x):
        ...
        for i, out in  enumerate([enc2x, enc4x, enc8x, enc16x, enc32x]):
            print(f"stage {i}, x.size {out.size()}")
